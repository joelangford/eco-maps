const animatePath = (pathPlots, map, google) => {
  let position = 0
  ...
  // Creates a bike marker that will move along the route
  ...
  // Creates a polyline of all the plots on the route, to display on click;
  ...
  // Moves the bike marker along the route at half second inteverals until it reaches it's destination
...
const calculateAndDisplayRoute = (google, directionsService, map, route) => {
  // Finds the route using the google directions api
  directionsService
    .route({
      origin: route.origin,
    ...
    .then((response) => {
      // Takes the overview_path from the route and creates an array of the plots
      const route = response.routes[0].overview_path
      const linePath = []
      route.forEach(location => {
        linePath.push({ lat: parseFloat(location.lat()), lng: parseFloat(location.lng()) })
      })
      animatePath(linePath, map, google)
    })
